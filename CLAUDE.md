...
- **The Today page is a paper agenda** (`src/components/home/HomeClient.tsx`): a fixed Mon–Sun `WeekStrip` on top, the open day (`AgendaDay`: schedule band, carried band on today only, one line per pinned tag with a blank slot, the daily note as a Notes margin) beneath, the context rail beside it. One fetch per week (`getAgendaWeekAction` via `useAgendaWeek`) feeds both the strip and the day. Past days render as a record.
