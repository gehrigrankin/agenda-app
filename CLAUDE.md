...
- **No local users table.** `ownerId` columns store an opaque owner string; every query must be owner-scoped. Signed-in owners are Clerk user ids (`user_…`); signed-out **guests** own a `guest_…` id held in an httpOnly cookie and get the full app. Resolve the owner with `getOwnerId()` / `requireOwnerId()` from `src/app/app/owner.ts` — never `auth()` directly in a page — and never assume an owner has a Clerk account. See the guest section in `CONTEXT.md`; signing up claims the guest's rows via `/app/claim`, and `/api/cron/purge-guests` sweeps abandoned ones after 30 days.
...
- **Bubbles are the folder system.** Bubbles opt in via `isFolder` and surface as sidebar folders; notes with a `bubbleId` live inside a bubble and are excluded from the main notes list. The `tags` table's hierarchy exists in the schema but has no folder-tree UI — tags shipped as FLAT labels on tasks instead (`#tag` in the quick-add, a per-row picker, and a tag filter in the Tasks rail; see `src/server/tags.ts`). Notes are not tagged yet. **Pinned tags are agenda lines**: `tags.isPinned` + `sortOrder` (written only via `setAgendaLines` / `appendAgendaLine`) are the ruled lines printed on every day of the Today page; a task lands on the first line whose tag it carries.
- **The Today page is a paper agenda** (`src/components/home/HomeClient.tsx`): a fixed Mon–Sun `WeekStrip` on top, the open day (`AgendaDay`: a schedule band, then the daily note as the page) beneath, and the rail beside it whose first panel is the `TasksWidget` with "Due today" printed on the pinned-tag lines. Tasks deliberately stay in the rail, not on the page. One fetch per week (`getAgendaWeekAction` via `useAgendaWeek`) draws the strip. Past days render as a record.
...
- **Putting a task on a note takes BOTH halves**: a `task` node in the note's serialized content *and* a `note_tasks` row. Use the `attachTaskToNote` helper in `mcp-tools.ts`, never `linkTaskToNote` alone — `reconcileNoteTasks` rederives the link set from note content on the next editor save, drops any link older than its 60s grace window with no matching node, then hard-deletes tasks left with zero links. A link-only attach silently destroys the user's task a minute later.
