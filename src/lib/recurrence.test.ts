...
function spec(
  overrides: Partial<RecurrenceSpec> & { freq: RecurrenceSpec["freq"] },
): RecurrenceSpec {
  return {
    weekday: null,
    intervalDays: null,
...
describe("nextOccurrence", () => {
  it("supports several weekdays in one weekly habit schedule", () => {
    const monFri = spec({ freq: "weekly", weekday: 1, weekdays: [1, 5] });
    expect(nextOccurrence(monFri, "2026-07-01", "2026-07-02")).toBe(
      "2026-07-03",
    );
    expect(nextOccurrence(monFri, "2026-07-01", "2026-07-04")).toBe(
      "2026-07-06",
    );
    expect(nextOccurrence(monFri, "2026-07-01", "2026-07-06")).toBe(
      "2026-07-06",
    );
  });

  it("keeps the legacy single weekday behavior when weekdays is absent", () => {
    expect(
      nextOccurrence(
        spec({ freq: "weekly", weekday: 5 }),
        "2026-07-01",
        "2026-07-04",
      ),
    ).toBe("2026-07-10");
  ...
  describe("daily", () => {
    ...
    it("returns fromDate when it is after the anchor", () => {
      expect(nextOccurrence(daily, "2026-01-01", "2026-03-05")).toBe(
        "2026-03-05",
      );
    });

    it("returns the anchor when fromDate is before it", () => {
      expect(nextOccurrence(daily, "2026-03-05", "2026-01-01")).toBe(
        "2026-03-05",
      );
    });

    it("returns the same day when fromDate equals the anchor", () => {
      expect(nextOccurrence(daily, "2026-01-01", "2026-01-01")).toBe(
        "2026-01-01",
      );
    });
  });
  ...
  describe("weekly", () => {
    ...
    it("advances to the next matching weekday", () => {
      ...
      expect(nextOccurrence(friday, "2026-01-01", "2026-01-01")).toBe(
        "2026-01-02",
      );
    });

    it("returns fromDate itself when it already falls on the weekday", () => {
      expect(nextOccurrence(friday, "2026-01-01", "2026-01-02")).toBe(
        "2026-01-02",
      );
    });
    ...
    it("wraps across the week boundary", () => {
      ...
      expect(nextOccurrence(friday, "2026-01-01", "2026-01-03")).toBe(
        "2026-01-09",
      );
    });

    it("starts from the anchor when fromDate is earlier", () => {
      expect(nextOccurrence(friday, "2026-07-06", "2026-01-01")).toBe(
        "2026-07-10",
      );
    });
  ...
  describe("interval", () => {
    ...
    it("phase-locks occurrences to the anchor", () => {
      ...
      expect(nextOccurrence(every3, "2026-01-01", "2026-01-05")).toBe(
        "2026-01-07",
      );
      expect(nextOccurrence(every3, "2026-01-01", "2026-01-02")).toBe(
        "2026-01-04",
      );
    });

    it("returns fromDate when it lands exactly on an occurrence", () => {
      expect(nextOccurrence(every3, "2026-01-01", "2026-01-04")).toBe(
        "2026-01-04",
      );
    });

    it("returns the anchor when fromDate is on or before it", () => {
      expect(nextOccurrence(every3, "2026-01-10", "2026-01-01")).toBe(
        "2026-01-10",
      );
      expect(nextOccurrence(every3, "2026-01-10", "2026-01-10")).toBe(
        "2026-01-10",
      );
    });
    ...
    it("stays phase-locked across month boundaries", () => {
      ...
      expect(nextOccurrence(every3, "2026-01-01", "2026-02-01")).toBe(
        "2026-02-03",
      );
    });
    ...
    it("returns null for a malformed interval", () => {
      expect(
        nextOccurrence(
          spec({ freq: "interval", intervalDays: null }),
          "2026-01-01",
          "2026-01-01",
        ),
      ).toBeNull();
      expect(
        nextOccurrence(
          spec({ freq: "interval", intervalDays: 0 }),
          "2026-01-01",
          "2026-01-01",
        ),
      ).toBeNull();
  ...
  describe("monthly", () => {
    ...
    it("returns the day in the current month when still ahead", () => {
      expect(nextOccurrence(day15, "2026-01-01", "2026-01-10")).toBe(
        "2026-01-15",
      );
      expect(nextOccurrence(day15, "2026-01-01", "2026-01-15")).toBe(
        "2026-01-15",
      );
    });

    it("rolls to the next month when the day has passed", () => {
      expect(nextOccurrence(day15, "2026-01-01", "2026-01-20")).toBe(
        "2026-02-15",
      );
    });

    it("clamps day 31 to the end of February", () => {
      expect(nextOccurrence(day31, "2026-01-01", "2026-02-01")).toBe(
        "2026-02-28",
      );
    });

    it("clamps day 31 to the end of February in a leap year", () => {
      expect(nextOccurrence(day31, "2028-01-01", "2028-02-01")).toBe(
        "2028-02-29",
      );
    });

    it("clamps day 31 to April 30", () => {
      expect(nextOccurrence(day31, "2026-01-01", "2026-04-01")).toBe(
        "2026-04-30",
      );
    });

    it("does not clamp in 31-day months", () => {
      expect(nextOccurrence(day31, "2026-01-01", "2026-03-01")).toBe(
        "2026-03-31",
      );
      expect(nextOccurrence(day31, "2026-01-01", "2026-01-31")).toBe(
        "2026-01-31",
      );
    });
    ...
    it("returns null for a malformed month day", () => {
      expect(
        nextOccurrence(
          spec({ freq: "monthly", monthDay: null }),
          "2026-01-01",
          "2026-01-01",
        ),
      ).toBeNull();
...
describe("dueOccurrence", () => {
  ...
  it("returns today for a fresh daily rule anchored today", () => {
    expect(dueOccurrence(daily, "2026-07-07", null, "2026-07-07")).toBe(
      "2026-07-07",
    );
  });

  it("returns today for a fresh daily rule anchored in the past", () => {
    expect(dueOccurrence(daily, "2026-07-01", null, "2026-07-07")).toBe(
      "2026-07-07",
    );
  });
  ...
  it("returns null when today's occurrence was already materialized", () => {
    expect(
      dueOccurrence(daily, "2026-07-01", "2026-07-07", "2026-07-07"),
    ).toBeNull();
  });
  ...
  it("returns todayStr for a daily rule dormant for 500+ days (regression)", () => {
    ...
    expect(dueOccurrence(daily, "2025-01-01", "2025-02-01", "2026-07-07")).toBe(
      "2026-07-07",
    );
  });
  ...
  it("carries exactly one occurrence for a dormant weekly rule", () => {
    ...
    const friday = spec({ freq: "weekly", weekday: 5 });
    expect(
      dueOccurrence(friday, "2026-01-02", "2026-02-06", "2026-07-07"),
    ).toBe("2026-07-03");
  });
  ...
  it("handles a dormant interval rule with a period longer than 31 days", () => {
    ...
    expect(
      dueOccurrence(every45, "2025-01-01", "2025-01-01", "2026-07-07"),
    ).toBe("2026-06-25");
  });
  ...
  it("handles a dormant monthly rule with clamping", () => {
    ...
    const day31 = spec({ freq: "monthly", monthDay: 31 });
    expect(dueOccurrence(day31, "2026-01-01", "2026-01-31", "2026-07-07")).toBe(
      "2026-06-30",
    );
  });
  ...
  it("returns null when lastDate is on or after the most recent occurrence", () => {
    ...
    expect(
      dueOccurrence(day15, "2026-01-01", "2026-06-15", "2026-07-07"),
    ).toBeNull();
    // Weekly: last done on today's occurrence.
    const tuesday = spec({ freq: "weekly", weekday: 2 });
    expect(
      dueOccurrence(tuesday, "2026-01-06", "2026-07-07", "2026-07-07"),
    ).toBeNull();
  });

  it("returns the occurrence strictly after lastDate when one is due", () => {
    const day15 = spec({ freq: "monthly", monthDay: 15 });
    expect(dueOccurrence(day15, "2026-01-01", "2026-05-15", "2026-07-07")).toBe(
      "2026-06-15",
    );
  });
});
...
describe("parseRecurrenceInput", () => {
  ...
  it('parses "every month on the 15th"', () => {
    const parsed = parseRecurrenceInput(
      "pay rent every month on the 15th",
      TODAY,
    );
    expect(parsed).toEqual({
      title: "pay rent",
  ...
  it('parses "1st of each month"', () => {
    const parsed = parseRecurrenceInput(
      "invoice clients 1st of each month",
      TODAY,
    );
    expect(parsed?.spec).toEqual({
      freq: "monthly",
  ...
  it("normalizes 12-hour edge times", () => {
    expect(
      parseRecurrenceInput("meds every day 12am", TODAY)?.spec.remindAt,
    ).toBe("00:00");
    expect(
      parseRecurrenceInput("lunch every day 12pm", TODAY)?.spec.remindAt,
    ).toBe("12:00");
  });
...
describe("recurrenceChipLabel", () => {
  it("labels each frequency", () => {
    ...
    expect(recurrenceChipLabel(spec({ freq: "weekly", weekday: 0 }))).toBe(
      "Sun",
    );
    expect(recurrenceChipLabel(spec({ freq: "weekly", weekday: 5 }))).toBe(
      "Fri",
    );
    expect(
      recurrenceChipLabel(spec({ freq: "interval", intervalDays: 3 })),
    ).toBe("3d");
    expect(recurrenceChipLabel(spec({ freq: "monthly", monthDay: 1 }))).toBe(
      "1st",
    );
  });

  it("uses correct ordinals", () => {
    expect(recurrenceChipLabel(spec({ freq: "monthly", monthDay: 2 }))).toBe(
      "2nd",
    );
    expect(recurrenceChipLabel(spec({ freq: "monthly", monthDay: 3 }))).toBe(
      "3rd",
    );
    expect(recurrenceChipLabel(spec({ freq: "monthly", monthDay: 11 }))).toBe(
      "11th",
    );
    expect(recurrenceChipLabel(spec({ freq: "monthly", monthDay: 12 }))).toBe(
      "12th",
    );
    expect(recurrenceChipLabel(spec({ freq: "monthly", monthDay: 13 }))).toBe(
      "13th",
    );
    expect(recurrenceChipLabel(spec({ freq: "monthly", monthDay: 21 }))).toBe(
      "21st",
    );
    expect(recurrenceChipLabel(spec({ freq: "monthly", monthDay: 22 }))).toBe(
      "22nd",
    );
    expect(recurrenceChipLabel(spec({ freq: "monthly", monthDay: 23 }))).toBe(
      "23rd",
    );
    expect(recurrenceChipLabel(spec({ freq: "monthly", monthDay: 31 }))).toBe(
      "31st",
    );
  });
});
...
describe("describeSchedule", () => {
  it("describes each frequency", () => {
    ...
    expect(describeSchedule(spec({ freq: "weekly", weekday: 0 }))).toBe(
      "Every Sunday",
    );
    expect(describeSchedule(spec({ freq: "interval", intervalDays: 3 }))).toBe(
      "Every 3 days",
    );
    expect(describeSchedule(spec({ freq: "monthly", monthDay: 1 }))).toBe(
      "1st of each month",
    );
  });
});
...
describe("toInputPhrase", () => {
  it("builds phrases for each frequency", () => {
    expect(toInputPhrase("Water plants", spec({ freq: "daily" }))).toBe(
      "Water plants every day",
    );
    expect(toInputPhrase("Gym", spec({ freq: "weekly", weekday: 5 }))).toBe(
      "Gym every friday",
    );
    expect(
      toInputPhrase("Stretch", spec({ freq: "interval", intervalDays: 3 })),
    ).toBe("Stretch every 3 days");
    expect(
      toInputPhrase("Pay rent", spec({ freq: "monthly", monthDay: 15 })),
    ).toBe("Pay rent 15th of each month");
  });
  ...
  it("appends the reminder time", () => {
    expect(
      toInputPhrase(
        "Pay rent",
        spec({ freq: "monthly", monthDay: 15, remindAt: "16:00" }),
      ),
    ).toBe("Pay rent 15th of each month 4:00pm");
  });
  ...
  it("round-trips through parseRecurrenceInput", () => {
    ...
    const weeklyParsed = parseRecurrenceInput(
      toInputPhrase("Review inbox", weekly),
      "2026-07-07",
    );
